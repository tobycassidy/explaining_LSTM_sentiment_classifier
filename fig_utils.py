...
train_proportions = go.Figure(
    go.Bar(x=x, y=y, marker=dict(color="#1F3F49", line=dict(color="white", width=3)))
)
...
location_count = {
    l: c
    for l, c in zip(
        df_train["Location"].value_counts().head(10).index,
...
for location in location_count.keys():
    if location == "London":
        location_df.append(
            df_train.loc[df_train["Location"] == location, :]
            .groupby("TweetAt")
