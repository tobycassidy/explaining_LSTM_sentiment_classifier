...
embedding = tf.keras.layers.Embedding(
    vocab_size + 1, embedding_dim, input_length=max_seqlen, name="embedding"
)
...
actual_pred_time.add_trace(
    go.Scatter(
        ...
        y=df_temp["Actual"],
        mode="lines+markers",
        marker=dict(color="#B3C100", line=dict(color="darkslategrey", width=1)),
        name="Actual",
        opacity=0.8,
    )
)
...
actual_pred_time.add_trace(
    go.Scatter(
        ...
        mode="lines+markers",
        marker=dict(color="#1F3F49", line=dict(color="darkslategrey", width=1)),
        name="Prediction",
        opacity=0.8,
    )
)
        ...
        dcc.Tabs(
            ...
            """,
                        ...
                        html.Div(
                            [
                                dcc.Graph(
                                    figure=fig_utils.train_proportions,
                                    style={
                                        "width": "45%",
                                        "margin-bottom": "24px",
                                        "margin-left": "10px",
                                ...
                                dcc.Graph(
                                    figure=fig_utils.test_proportions,
                                    style={
                                        "width": "45%",
                                        "margin-bottom": "24px",
                                        "margin-left": "10px",
            ...
            """,
                ...
                dcc.Tab(
                    label="Preprocessing and Sentiment Classification",
                    style=tab_style,
                    selected_style=tab_selected_style,
                    children=[
                        dcc.Markdown(
            ...
            """,
                        ...
                        html.Div(
                            [
                                ...
                                html.Img(
                                    src="data:image/png;base64,{}".format(
                                        encoded_horizontal_arrow.decode()
                                    ),
                                    height="75 px",
                                    width="auto",
            ...
            """,
                        ...
                        dcc.Graph(
                            id="actual-vs-prediction-graphic",
                            style={
                                "width": "83.5%",
                                "marginBottom": 5,
                                "marginTop": 15,
...
def update_output(value):
    actual = pd.DataFrame(y_test).rename(
        columns={
    ...
    pred = pd.DataFrame(np.round(model.predict(X_test), 0)).rename(
    ...
    fig = go.Figure(
        go.Bar(
            x=x,
            y=y,
            marker=dict(color="#1F3F49", line=dict(color=line_colors, width=3)),
        )
    )
    ...
    for x_pos, y_pos, percent in zip(x, y, percentages):
        annotations.append(
            dict(x=x_pos, y=y_pos + 25, text=f"{percent}", showarrow=False)
        )
        ...
        dcc.Tabs(
            ...
            """,
                        ...
                        html.Div(
                            [
                                dcc.Graph(
                                    figure=train_proportions, style={"width": "50%"}
                                ),
                                dcc.Graph(
                                    figure=test_proportions, style={"width": "50%"}
                                ),
                            ],
                            style={"display": "flex"},
                        ),
